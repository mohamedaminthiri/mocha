 ...
 * @param {boolean} [opts.changed] - If `true`, run only changed test files
 * @param {Object} fileCollectParams - Parameters that control test
 *   file collection. See `lib/cli/collect-files.js`.
 * @returns {Promise<Runner>}
 * @private
 */
const singleRun = async (mocha, {exit, changed}, fileCollectParams) => {
  let files = collectFiles(fileCollectParams);
  debug('found %d test files', files.length);
  if (changed) {
    const moduleMap = ModuleMap.create({entryFiles: files});
    files = moduleMap.filterEntryFiles(
      moduleMap.findAffectedFiles(moduleMap.getChangedFiles())
    );
    debug('--changed requested; running %d test files', files.length);
  }
  mocha.files = files;
